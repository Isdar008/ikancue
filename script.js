...
async function loadStatusFromApi() {
  try {
    ...
    const saldoEl = document.getElementById("statusSaldo");
    const totalAkunEl = document.getElementById("statusTotalAkun");
    const levelEl = document.getElementById("statusLevel");

    if (!email) {
      if (saldoEl) saldoEl.textContent = "Rp 0";
      if (totalAkunEl) totalAkunEl.textContent = "0";
      if (levelEl) levelEl.textContent = "MEMBER";
      return;
    }
    ...
    const d = json.data;
    if (saldoEl) saldoEl.textContent = fmtRupiah(d.saldo);
    if (totalAkunEl) totalAkunEl.textContent = d.totalAkun || 0;
    if (levelEl)
      levelEl.textContent = (d.level || "member").toUpperCase();
  } catch (e) {
...
// ===== RENDER SERVER CARD V2 (TANPA TOTAL AKUN) =====
function renderServers() {
  ...
  if (!cachedServers.length) {
    wrap.innerHTML =
      "<p class='muted small'>Server kosong / gagal memuat.</p>";
    return;
  }
  ...
  wrap.innerHTML = cachedServers
    .map((s) => {
      ...
      return `
        <div class="server-card server-card-v2">
          <div class="server-card-header">
            <div>
              <div class="server-name">${s.nama_server}</div>
              <div class="server-meta-small">${s.lokasi} • ${s.isp}</div>
            </div>
            <span class="badge ${statusClass} server-status-pill">
              ${statusText}
            </span>
          </div>
          ...
          <div class="server-card-body">
            <div class="server-chip">
              Harga ${fmtRupiah(s.harga)}/hari
            </div>

            <div class="server-spec-wrap">
              <div class="server-spec">
                <span>Kuota / hari</span>
                <strong>${s.quota} GB</strong>
              </div>
              <div class="server-spec">
                <span>IP Max</span>
                <strong>${s.iplimit}</strong>
              </div>
            </div>
          </div>
        </div>
...
if (btnAdminAddSaldo) {
  btnAdminAddSaldo.onclick = async (ev) => {
    ...
    const targetEmail = (document.getElementById("adminTargetUser").value || "")
      ...
      .toLowerCase();
    const amount =
      parseInt(document.getElementById("adminAmount").value, 10) || 0;
...
if (btnLogout) {
  btnLogout.onclick = () => {
    ...
    showPage("login"); // balik ke form login
  };
}
...
if (modalCopyMain) {
  modalCopyMain.onclick = async () => {
    ...
    const linkEls = bodyEl.querySelectorAll(".scroll-x"); // semua baris link
  ...
  btnCreate.onclick = async (ev) => {
    ...
    try {
      ...
      showResultModal(
        ...
        `Server: ${json.data.serverName} • Harga: ${fmtRupiah(
          json.data.totalHarga
        )}`,
        json.data.message,
        type // kirim jenis akun
      );
...
if (btnRenew) {
  btnRenew.onclick = async (ev) => {
    ...
    const serverId = parseInt(
      document.getElementById("renewServer").value
    );
    const username = document
      .getElementById("renewUser")
      .value.trim();
    const days = parseInt(
    ...
    if (!serverId || !username || !days || !email)
      return alert(
        "Lengkapi semua form & pastikan kamu sudah login."
      );
...
if (btnTopup) {
  btnTopup.onclick = async (ev) => {
    ...
    if (!amount || amount < 1000)
      return alert("Minimal topup Rp 1.000");
    ...
    try {
      ...
      if (card && img && infoEl && linkEl) {
        infoEl.textContent = `Invoice: ${
          data.orderId || "-"
        } • Nominal: ${fmtRupiah(data.amount || amount)}`;
...
async function loadTopupHistory() {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/topup-history?email=${encodeURIComponent(
        email
      )}`
    );
    const json = await res.json();
      ...
      .map((item) => {
        ...
        return `
          <li>
            #${item.id || "-"} • ${fmtRupiah(
          item.amount
        )} • ${statusText}${refText}${methodText} • ${waktu}
          </li>
        `;
      })
      .join("");
